...
class Track:
    ...
    def _load_fit_data(self, fit: dict):
        ...
        if self.type == "running": 
            ...
            self.moving_dict["set_mesgs"] = fit["set_mesgs"]
    ...
    def to_message(self):
        duration = format_time(self.moving_dict["moving_time"])
        if self.type.lower() == "running":
            distance = self.moving_dict["distance"]
            pace = self.moving_dict["average_speed"]
            message = f"Just finished running - Distance: {distance} km, Duration: {duration}, Pace: {pace}"
        elif self.type.lower() == "training":
            message = generate_message(self.moving_dict["set_mesgs"])
        else:
            message = f"Just finished {self.type} - Spent {duration} minutes. Burned {self.moving_dict['calories']} calories"

        return message

def generate_message(training_data):
    categories = set(entry["category"].lower() for entry in training_data if "category" in entry)
    if len(categories) == 1:
        category = categories.pop()
        total_repetitions = sum(entry.get("repetitions", 0) for entry in training_data if entry.get("category", "").lower() == category)
        message = f"Just completed {total_repetitions} {category.replace('_', '-')}"
        if category == "push_up":
            message = f"Just completed {total_repetitions} push-ups"
    else:
        summary = {}
        for entry in training_data:
            if "category" in entry:
                ...
                if category not in summary:
                    summary[category] = repetitions
                else:
                    summary[category] += repetitions
        
        message = "Just completed:\n"
        for category, repetitions in summary.items():
            formatted_category = category.replace('_', '-')
            if category == "push_up":
                formatted_category = "push-ups"
            message += f"- {formatted_category.capitalize()}: {repetitions} repetitions\n"
    
    return message.strip()  # Strip any leading/trailing whitespace
